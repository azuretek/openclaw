...
describe("config model validation reporting", () => {
  it("reports a plugin-provided ref's resolver reason for a newly added agent fallback", async () => {
    ...
    // reference", which left a plugin-provided provider with an unregistered model
    // catalog indistinguishable from a missing credential.
    const reason =
      'Unknown model: deepseek/deepseek-v4-flash. Found agents.defaults.models["deepseek/deepseek-v4-flash"], but no matching models.providers["deepseek"].models[] entry.';
    ...
    expect(result.errors[0]).toContain('"deepseek/deepseek-v4-flash"');
