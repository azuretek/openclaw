...
- Identity-scope-only config snapshots advance the projection's current config while retaining unchanged session facts. Keep permission revocation and catalog publications independent. A config publication refreshes every row unless it is validated against the previous publication's record, meaning its value fingerprint and serialized resolution facts; that comparison is what catches an in-place edit or changed resolution provenance. Object identity cannot make that check, so a same-object publication refreshes unless its publisher validates it against the record, as the source-only republish in `setRuntimeConfigSourceSnapshotIfCurrent` does. Forced invalidation always refreshes.
