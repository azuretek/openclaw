...
- Identity-scope-only config snapshots advance the projection's current config while retaining unchanged session facts. Keep permission revocation and catalog publications independent; changed resolution provenance, forced invalidation, and same-object publications retain broad refresh behavior. The one same-object exception is a source-only republish through `setRuntimeConfigSourceSnapshotIfCurrent` whose runtime object still hashes to the fingerprint recorded at its last publication and whose serialized resolution facts the copy leaves unchanged: rows read only the runtime object, so that publication changes no row input and emits no session change.
